...
the repository's git hooks, and the release gate can pass again.

- **Ten releases failed in a row for a reason no local gate could see.** The
  packaged VSIX included `.husky/**`, whose `_` directory is generated by
  `npx husky` and gitignored. It exists on a developer's machine and never on a
  fresh CI checkout, so the committed artifact and the one the release workflow
  rebuilds differed by nineteen files. The reproducibility check was right to
  refuse; locally the two builds agreed with each other, so nothing was red.
- `.husky/**` and `.vscode-test/**` are excluded, and `package:audit` now runs
  `vsce ls` and refuses any packaged path inside a dot-directory. Both
  assertions were shown to fail on the removed exclusion and pass with it.
