...
| F052 | Managed organization settings                       | SHIPPED in 0.81.0, was BLOCKED | `src/core/policy-v2.ts`, `src/backend/organization-policy-client.ts`, `apps/claw-agent-service/src/modules/fleet/`    | Unblocked by building the backend contract that did not exist: an organization policy, intersected across memberships, fetched with the account data and enforced in the tool evaluator. Tighten-only and unsigned, for the reason project rules are. Still open: `allowedModels` in the model picker and `minimumPermissionMode` in the configuration clamp, neither of which an invocation carries.                           |
