...
| F072 | VS Code URI handler                | CONFLICT, audit corrected | `tests/extension-host/index.cjs:35`, `CHANGELOG.md:1470`, `docs/AUTHENTICATION.md:29`                      | Not missing — deliberately absent. The custom URI callback was replaced by a state-validated one-shot loopback callback, and a host test asserts no `onUri` activation event remains. Any URI surface reverses that decision and needs an ADR first, because a `vscode://` link is triggerable by any web page. |
| F073 | Deep links                         | CONFLICT, audit corrected | same as F072                                                                                               | Blocked behind the same decision: there is no inbound scheme to hang deep links on until F072 is settled.                                                                                                                                                                                                       |
