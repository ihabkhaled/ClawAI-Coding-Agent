...
| F057 | Checkpoints and rewind             | PARTIAL | `src/services/file-transaction-service.ts:163`, `src/extension.ts:87`                                      | One-step code undo only; no named checkpoints, no conversation fork.                     |
...
| F059 | Conversation rewind command        | MISSING | `src/services/conversation-session-service.ts:47`                                                          | No pre-clear snapshot, no restore command.                                               |
...
| F072 | VS Code URI handler                | MISSING | `src/extension.ts` has no `registerUriHandler`                                                             | No `vscode://` entry point.                                                              |
| F073 | Deep links                         | MISSING | `src/services/browser-authorization-service.ts:160`                                                        | Outbound `openExternal` only; no inbound scheme.                                         |
| F074 | Session recaps                     | MISSING | `src/services/conversation-session-service.ts:47`                                                          | Raw message replay; no return-to-session summary.                                        |
| F075 | Transcript export                  | PARTIAL | `src/services/run-journal-service.ts:127`, `src/services/evidence-bundle-service.ts:161`                   | Exports journal metadata and evidence zips, not the transcript; tool-only.               |
...
- **F072/F073** → `src/extension.ts` `activate` registers `registerUriHandler`,
  routing into `conversation-session-service.ts:35` `openChat`, validated by a Zod
  schema beside `chat-inbound-message.ts`; signing follows
  `src/core/loopback-authorization.ts`.
...
4. **F072 gates F073.** No URI handler is registered at all.
