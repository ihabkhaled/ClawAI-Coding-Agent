...
| F003 | Glob and Grep                | PARTIAL | `src/infrastructure/vscode-filesystem-tool-executor.ts:344`, `:330`, `src/infrastructure/workspace-scan.constants.ts:21`                                | `search` is a literal substring test: no regex, multiline, case-insensitivity, context lines, type scoping or per-call exclude. `findFiles` passes no excludes, so the files tool ignores `.gitignore`. |
...
| F016 | AskUserQuestion              | PARTIAL | `src/core/approval-broker.ts:52`, `src/webview/chat-public-state.ts:137`                                                                                | Approval is the only interrupt channel and it is binary: no choice sets, free text, validation, timeout policy or agent-callable question tool.                                                         |
...
| F020 | LSP tool                     | MISSING | `src/infrastructure/intelligence-tool-executor.ts:18`                                                                                                   | `workspace.intelligence` is a custom symbol graph, not the language server: no definition, references, hover or call hierarchy.                                                                         |
| F021 | IDE diagnostics              | MISSING | `src/core/quality-graph.ts:147` parses diagnostics out of gate stdout                                                                                   | Nothing reads the VS Code Problems collection.                                                                                                                                                          |
...
| F031 | EndConversation safeguard    | PARTIAL | `src/core/runtime/runtime-tool-contracts.ts:112`, `src/services/runtime-run-service.ts:383`                                                             | `final` ends a turn, chosen per envelope. No agent-callable terminal action, no evidence flush, and nothing blocks ending while `ApprovalBroker.pending` is non-empty.                                  |
