...
| F104 | Code review and multi-agent review                     | PARTIAL, narrowed in 0.73.0 | ext     | `src/core/findings.ts`, `src/services/workflow-service.ts:46`, `BE/.../code-review.handler.ts:11`                                                                   | The finding type, dedupe and aggregation now exist and are surfaced. Still open on the extension side: attaching findings to `SubAgentOutcome` so reviewer sub-agents report through them rather than in prose. Still open on the backend: `CodeReviewHandler` throws `SCAFFOLD-R3` and has no references. |
