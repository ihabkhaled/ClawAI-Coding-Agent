...
| F104 | Code review and multi-agent review                     | PARTIAL, narrowed in 0.73.0 and 0.74.0 | both    | `src/core/findings.ts`, `src/services/sub-agent-findings-observer.ts`, `BE/.../code-review.handler.ts:11`                                                           | Extension side is done: reviewer sub-agents report structured findings, several reviewers merge into one list, and a release verdict falls out. Still open on the backend: `CodeReviewHandler` throws `SCAFFOLD-R3` and has no references, so a server-side review lane does not exist. |
