...
Companion to the ClawAI platform program _Billing & Subscription Hardening v2_
...
Per `CLAUDE.md`: _the backend owns entitlements and quota; the extension owns UX._ This
program does not weaken that. The extension gains **accurate preflight feedback**, never
authority. A cached snapshot may only ever _narrow_ what the user can attempt — it can
