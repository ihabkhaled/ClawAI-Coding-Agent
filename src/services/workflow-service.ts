...
export function buildEditPlanRepairPrompt(previousResponse: string): string {
  return [
    ...
    'Use only safe relative workspace paths. Create and update require full file content. Commands are optional, bounded development tools with no chaining or redirection.',
    '<previous-response>',
    previousResponse,
    '</previous-response>',
  ].join('\n');
